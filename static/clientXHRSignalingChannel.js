...
function connect(failureCB) {
  var failureCB = function(s) {console.log(s);};
  ...
  client.open("GET", "https://daisyedu.cn:8081/connect?key=" + key);
  client.send();
}
...
function get(getResponseHandler) {
  ...
  var client = new XMLHttpRequest();
  client.onreadystatechange = handler;
  client.open("POST", "https://daisyedu.cn:8081/get");
...
function send(msg, responseHandler) {
  ...
  var client = new XMLHttpRequest();
  client.onreadystatechange = handler;
  client.open("POST", "https://daisyedu.cn:8081/send");
